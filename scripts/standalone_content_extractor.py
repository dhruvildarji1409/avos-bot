#!/usr/bin/env python3
"""
Standalone Confluence Content Extractor

This script extracts and processes content from Confluence pages
to provide answers to user queries about AVOS.
It uses the standalone_confluence_client.py which makes direct API calls to Confluence.
"""
import os
import sys
import json
...
try:
    from standalone_confluence_client import search_confluence, get_page_content
except ImportError:
    print("Error: standalone_confluence_client.py not found in the same directory.")
    sys.exit(1)
...
def search_and_extract_content(query, limit=5):
    ...
    for result in search_results:
        ...
        structured_content['url'] = page.get('url', '')
        structured_content['space'] = page.get('metadata', {}).get('spaceKey', 'Unknown')
...
def main():
    parser = argparse.ArgumentParser(description="Standalone Confluence Content Extractor")
