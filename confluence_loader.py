...
# Import our new OpenAI authentication module
sys.path.append(os.path.dirname(os.path.abspath(__file__)) + '/src')
from openai_auth import create_client, DEPENDENCIES_INSTALLED
...
class HeaderBasedChunker:
    ...
    def _extract_text_with_code(self, element) -> str:
        ...
        for child in element.children:
            ...
            elif child.name in ['table', 'img']:  # Preserve tables and images
                if hasattr(child, 'prettify'):
                    texts.append(child.prettify())
                else:
    ...
    def chunk_content(self, html_content: str, page_id: str) -> List[Dict]:
        ...
        Enhanced to ensure entire blocks are preserved.
        """
        soup = BeautifulSoup(html_content, 'html.parser')
        chunks = []
        
        # Find all headers and content elements
        all_elements = soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'div', 'ul', 'ol', 'table', 'pre', 'code', 'img'])
        ...
        for element in all_elements:
            ...
            if element.name in self.header_tags:
                ...
                if current_chunk and chunk_content:
                    ...
                    # Store the full context including header and content for embedding
...
class ConfluenceMongoDB:
    ...
    def __init__(
        ...
        self.collection.create_index([("parent_chunk_id", pymongo.ASCENDING)])
        self.collection.create_index([("children", pymongo.ASCENDING)])
        self.collection.create_index([("page_id", pymongo.ASCENDING)])
        self.collection.create_index([("header", pymongo.ASCENDING)])
        self.collection.create_index([("depth", pymongo.ASCENDING)])
    ...
    def graph_based_retrieve(
        ...
        Enhanced to ensure entire blocks are returned with better context.
        ...
        Args:
            ...
            return_full_context: Whether to return the full context of chunks
        ...
        for doc, score in docs_with_scores:
            if score >= similarity_threshold:
                seed_nodes.append(doc["_id"])
                if len(seed_nodes) >= max_results:
                    break
        ...
        for node_id in all_relevant_nodes:
            ...
            if not doc:
                # If not found in the scored docs, fetch it directly
                doc = self.collection.find_one({"_id": node_id})
                if not doc:
                    continue
        ...
        for doc, score in graph_docs[:max_results]:
            ...
            result = {
                ...
                "header": doc["header"],
                "content": doc["content"] if return_full_context else doc["content"][:300] + "...",
            ...
            # Include full context if requested - this ensures the entire content block is available
            if return_full_context:
                result["full_context"] = doc.get("full_context", "")
    ...
    def search_by_query(self, query_text, openai_client=None, top_k=5):
        """
        Search the database using a text query.
        
        Args:
            query_text: The text query
            openai_client: Optional OpenAI client for generating embeddings
        ...
        """
        try:
            # Generate embedding for the query if OpenAI client is provided
            if openai_client:
                ...
                # Use graph-based retrieval with the embedding
                return self.graph_based_retrieve(
                    query=query_text,
                ...
                # Fallback to basic text search if no embedding client
                regex_pattern = '|'.join([re.escape(term) for term in query_text.split()])
                results = list(self.collection.find(
                    {"$or": [
                        {"header": {"$regex": regex_pattern, "$options": "i"}},
...
#######################################
def main():
    # Get OpenAI client using our authentication module
    ...
    for url in urls:
        try:
            ...
            # Increased max_depth from 2 to 3 for better recursion
            pages = loader.process_page_recursive(url, current_depth=0, max_depth=3)
            ...
            for page in pages:
                if page['id'] not in processed_pages:
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        # Use our OpenAI client to generate embeddings
                        # Increasing context length for more comprehensive embeddings
                        context = chunk["full_context"]
                        
                        # Create embeddings with increased context
                        try:
                            embedding_response = openai_client.embeddings.create(
                                model="text-embedding-ada-002",
                                input=context,
                                dimensions=1536  # Use full dimensions for better accuracy
                            )
